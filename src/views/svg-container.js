...
customElements.define('svg-container', class extends HTMLElement {
    ...
    constructor({options}) {
        ...
        if ( super() ) {
            ...
            this.setHTMLUnsafe(/* html */`
                <svg id=${ options.id || 'svg-container' } ></svg>
            `);
    ...
    connectedCallback(){
        ...
        this.children.namedItem('svg-container').setAttribute('style', /* style */`width:inherit; height: inherit;`)
        ...
        this.children.namedItem('svg-container').setAttribute('viewbox', `${0} ${0} ${window.innerWidth} ${window.innerHeight}`);
...
export default customElements.get('svg-container');